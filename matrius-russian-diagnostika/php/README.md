...
Тонкая прокладка для лендинга `/matrius-russian-diagnostika/`. Принимает JSON от React-формы и создаёт сделку в GC через snippet [`getcourse.php`](getcourse.php) (взят из anthropic-skill `getcourse-api`).
...
├── submit.php      # обработчик формы (POST /matrius-russian-diagnostika/php/submit.php)
...
1. На сервере перейти в каталог лендинга и клонировать/спулить репо так, чтобы папка `php/` лежала рядом с `index.html` лендинга:
   ```
   /var/www/web.matrius.online/matrius-russian-diagnostika/
   ├── index.html
   ├── assets/
   └── php/
       ├── submit.php
...
# Логи
tail -f /var/www/web.matrius.online/matrius-russian-diagnostika/php/logs/leads.log
```
