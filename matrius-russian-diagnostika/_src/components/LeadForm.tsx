...
// Аналитика: те же ID, что в layout.tsx
const YM_ID = 98858030;
const TMR_ID = '3743427';
const GOAL_LEAD_SUBMIT = 'lead_submit';

// Типизация для глобальных функций счётчиков, чтобы TS не ругался.
type YMFn = (id: number, action: string, goal?: string, params?: Record<string, unknown>) => void;
type TMRPush = (event: Record<string, unknown>) => void;
type TMRWindow = { _tmr?: { push: TMRPush } | Array<Record<string, unknown>> };
type WindowWithCounters = Window & TMRWindow & { ym?: YMFn };

function trackLeadSuccess(params?: Record<string, unknown>) {
  if (typeof window === 'undefined') return;
  const w = window as WindowWithCounters;
  try {
    if (typeof w.ym === 'function') {
      w.ym(YM_ID, 'reachGoal', GOAL_LEAD_SUBMIT, params);
    }
  } catch {
    /* счётчик мог быть заблокирован адблоком — лидам это не мешает */
  }
  try {
    // _tmr может быть массивом до загрузки скрипта (см. инициализацию в layout.tsx)
    // или объектом с push после. У обоих есть метод push().
    const tmr = w._tmr;
    if (tmr && typeof (tmr as { push: TMRPush }).push === 'function') {
      (tmr as { push: TMRPush }).push({
        type: 'reachGoal',
        id: TMR_ID,
        goal: GOAL_LEAD_SUBMIT,
      });
    }
  } catch {
    /* аналогично */
...
export default function LeadForm({ variant = 'light', submitLabel = 'Записаться', onSuccess }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(true);
      // Отправляем цель «lead_submit» в Яндекс.Метрику и Top.Mail.Ru
      // только при подтверждённой успешной отправке (res.ok && data.ok).
      trackLeadSuccess({
        offer_code: OFFER_CODE,
        utm_source: utm['utm_source'] || DEFAULT_UTM_SOURCE,
      });
