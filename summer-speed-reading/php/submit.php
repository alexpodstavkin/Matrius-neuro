<?php
/**
 * Обработчик формы лендинга /summer-speed-reading/.
 ...
 *
 * Отличия от /skorochtenie-neuro/php/submit.php:
 *   • поле «age» НЕобязательно (на этом лендинге нет селекта возраста)
 *   • offer_code и UTM ведут себя так же
 *
 * Установка:
 *   1. Скопировать .env.example → .env, заполнить GC_*.
 *   2. На фронте URL обработчика: /summer-speed-reading/php/submit.php
 *
 * Безопасность:
...
// ─────────────────────────────────────────────────────────────────────
// 5. Валидация (возраст НЕ требуется)
// ─────────────────────────────────────────────────────────────────────
...
$dealComment = "Лендинг: Лето без отупения (summer-speed-reading)";
...
$user = [
    ...
    'last_name'  => $lastName,
    'addfields'  => ['Источник' => 'summer-speed-reading'],
];
...
$dealNumber = 'sumr-' . substr(sha1(strtolower($email) . '|' . $offer . '|' . intdiv(time(), 60)), 0, 10);
